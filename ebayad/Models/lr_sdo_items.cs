//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace ebayad.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class lr_sdo_items
    {
        public int ID { get; set; }
        public Nullable<int> LR_SDO_TABLE_ID { get; set; }
        public string PAYROLL_NO { get; set; }
        public string PAYROLL_NAME { get; set; }
        public string RESPO { get; set; }
        public Nullable<double> AMOUT { get; set; }
        public Nullable<double> ACTUAL { get; set; }
        public Nullable<double> DIFF { get; set; }
        public Nullable<double> FINAL_AMOUNT { get; set; }
        public Nullable<double> FINAL_ACTUAL { get; set; }
        public Nullable<double> FINAL_DIFF { get; set; }
    }
}
